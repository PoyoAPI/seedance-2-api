...
Focused server-side examples for building with `seedance-2`, `seedance-2-fast`, and `seedance-2-mini` on PoYo.
...
Use `seedance-2-mini` when you want a lower-cost draft loop before deciding which video direction is worth polishing.
...
- Lower-cost draft workflows with `seedance-2-mini`
...
Use `seedance-2-mini` when the goal is to compare several lower-cost video directions before polishing one.
